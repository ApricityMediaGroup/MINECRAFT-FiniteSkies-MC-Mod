...
import finiteskies.fsmod.dimension.EmpyreanTeleporter;
...
public class EmpyreanPortal extends BlockBreakable
{
    ...
    public EmpyreanPortal()
    {
        super(Material.PORTAL, false);
        this.setDefaultState(this.blockState.getBaseState().withProperty(AXIS, EnumFacing.Axis.X));
        this.setTickRandomly(true);
		setUnlocalizedName(Reference.FiniteBlocks.EMPYREANPORTAL.getUnlocalizedName());
		setRegistryName(Reference.FiniteBlocks.EMPYREANPORTAL.getRegistryName());
    }
    ...
    {
        EmpyreanPortal.Size blockportal$size = new EmpyreanPortal.Size(worldIn, pos, EnumFacing.Axis.X);

        if (blockportal$size.isValid() && blockportal$size.portalBlockCount == 0)
        {
            blockportal$size.placePortalBlocks();
            System.out.println("Portal result 0");
            return true;
        }
        else
        {
            EmpyreanPortal.Size blockportal$size1 = new EmpyreanPortal.Size(worldIn, pos, EnumFacing.Axis.Z);

            if (blockportal$size1.isValid() && blockportal$size1.portalBlockCount == 0)
            {
                blockportal$size1.placePortalBlocks();
                System.out.println("Portal result 1");
                return true;
            }
            else
            {
            	System.out.println("Portal result 2");
            	return false;
            }
        }
    }
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn)
    {
        if (!entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn.isNonBoss() && entityIn instanceof EntityPlayerMP)
        {
            ...
            } else if (thePlayer.dimension != EmpyreanDimension.DIMENSION_ID) {
            	thePlayer.timeUntilPortal = 10;
            	thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, EmpyreanDimension.DIMENSION_ID, new EmpyreanTeleporter(thePlayer.mcServer.worldServerForDimension(EmpyreanDimension.DIMENSION_ID)));
            } else {
            	thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, 0, new EmpyreanTeleporter(thePlayer.mcServer.worldServerForDimension(0)));
            }
        }
    }
    ...
    public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_)
    {
        ...
        EmpyreanPortal.Size blockportal$size = new EmpyreanPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.X);
        LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);

        if (!blockportal$size.isValid())
        {
            enumfacing$axis = EnumFacing.Axis.X;
            blockportal$size = new EmpyreanPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.Z);
        }
    ...
    public static class Size
        {
            ...
            protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_)
            {
                ...
                for (i = 0; i < 22; ++i)
                {
                    ...
                    if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock()) || this.world.getBlockState(blockpos.down()).getBlock() != ModBlocks.glowquartz)
                    {
                        break;
                    }
                }
                ...
                return block == ModBlocks.glowquartz ? i : 0;
            }
            ...
            protected int calculatePortalHeight()
            {
                ...
                {
                    if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != ModBlocks.glowquartz)
                    {
                        this.height = 0;
                        break;
                    }
