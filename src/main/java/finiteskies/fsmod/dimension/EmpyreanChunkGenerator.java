...
public class EmpyreanChunkGenerator implements IChunkGenerator
{
   ...
   // private final MapGenEndCity endCityGen = new MapGenEndCity(this);
    ...
    public EmpyreanChunkGenerator(World worldObjIn, boolean mapFeaturesEnabledIn, long seed)
    {
            ...
            if (this.getIslandHeightValue(x, z, 1, 1) > 40.0F)
            {
                ...
                for (int k = 0; k < j; ++k)
                {
                    ...
                    if (j1 > 0)
                    {
                        ...
                        if (this.worldObj.isAirBlock(blockpos.add(l, k1 + 1, i1)) && this.worldObj.getBlockState(blockpos.add(l, k1, i1)).getBlock() == Blocks.STONE)
                        {
